"use client";

import { Fragment } from "react";
import Link from "next/link";
import { usePathname, useSearchParams } from "next/navigation";
import ThemeToggle from "./ThemeToggle";
import styles from "./Nav.module.css";

const TABS = [
  { id: "work", label: "work" },
  { id: "hobbies", label: "hobbies" },
] as const;

export const LINKS = {
  github: "https://github.com/Jaaaayden",
  linkedin: "https://www.linkedin.com/in/jayden-c-le",
  resume: "/resume.pdf",
};

export default function Nav() {
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const activeTab =
    pathname === "/" ? (searchParams.get("tab") ?? "work") : null;

  return (
    <header className={styles.nav}>
      <div className={styles.inner}>
        <Link href="/" className={`${styles.wordmark} mono`}>
          jayden le
        </Link>

        <nav className={styles.tabs} aria-label="Sections">
          {TABS.map((tab) => (
            <Fragment key={tab.id}>
              <Link
                href={`/?tab=${tab.id}`}
                scroll={false}
                className={`${styles.tab} ${
                  activeTab === tab.id ? styles.tabActive : ""
                }`}
                aria-current={activeTab === tab.id ? "page" : undefined}
              >
                {tab.label}
              </Link>
              <span className={styles.dot} aria-hidden>
                ·
              </span>
            </Fragment>
          ))}
          <a
            href={LINKS.resume}
            target="_blank"
            rel="noreferrer"
            className={styles.tab}
          >
            resume
            <svg
              className={styles.ext}
              viewBox="0 0 10 10"
              width="8"
              height="8"
              aria-hidden
            >
              <path
                d="M2.2 7.8 L7.8 2.2 M3.4 2.2 H7.8 V6.6"
                stroke="currentColor"
                strokeWidth="1.3"
                fill="none"
                strokeLinecap="round"
              />
            </svg>
          </a>
        </nav>

        <div className={styles.links}>
          <a
            href={LINKS.github}
            target="_blank"
            rel="noreferrer"
            className={styles.iconLink}
            aria-label="GitHub"
          >
            <svg viewBox="0 0 16 16" width="18" height="18" fill="currentColor">
              <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8Z" />
            </svg>
          </a>
          <a
            href={LINKS.linkedin}
            target="_blank"
            rel="noreferrer"
            className={styles.iconLink}
            aria-label="LinkedIn"
          >
            <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
              <path d="M20.45 20.45h-3.55v-5.57c0-1.33-.03-3.04-1.85-3.04-1.86 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.47-.9 1.63-1.85 3.36-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29ZM5.34 7.43a2.06 2.06 0 1 1 0-4.12 2.06 2.06 0 0 1 0 4.12ZM7.12 20.45H3.56V9h3.56v11.45ZM22.22 0H1.77C.79 0 0 .77 0 1.72v20.55C0 23.23.79 24 1.77 24h20.45c.98 0 1.78-.77 1.78-1.73V1.72C24 .77 23.2 0 22.22 0Z" />
            </svg>
          </a>
          <ThemeToggle />
        </div>
      </div>
    </header>
  );
}
